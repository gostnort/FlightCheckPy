...
def process_and_add_hbprs(uploaded_file):
    ...
    try:
        ...
        # 步骤1：检测文件类型
        from ui.common import detect_file_type
        file_type = detect_file_type(file_content)       
        if file_type == 'UNKNOWN':
            st.error("❌ 无法识别文件类型，文件应包含HBPR或PR命令")
            return       
        # 步骤2：根据文件类型处理
        if file_type == 'HBPR':
            st.info("📄 检测到HBPR文件")
            process_hbpr_file(file_content, db)
        elif file_type == 'PR':
            st.info("📄 检测到PR文件")
            process_pr_file(file_content, db)           
...
def process_hbpr_file(file_content, db):
    """处理HBPR文件""" 
    with st.spinner("正在处理HBPR文件..."):
        ...
        if not processor.flight_data or flight_id_from_file not in processor.flight_data:
            ...
            return
        # 处理记录
        process_records_into_database(db, processor, flight_id_from_file)


def process_pr_file(file_content, db):
    """处理PR文件"""
    with st.spinner("正在处理PR文件..."):
        # 处理混合命令，将PR转换为HBPR
        ...
        # 使用HbprProcessor处理转换后的HBPR
        conn = db.get_connection()
        processor = HbprProcessor(conn)
        processor.parse_file_content(file_content)
        if not processor.flight_data or flight_id_from_file not in processor.flight_data:
            st.error("❌ HBPR处理失败，未找到有效记录")
            return
        # 处理记录
        process_records_into_database(db, processor, flight_id_from_file)


def process_records_into_database(db, processor, flight_id_from_file):
    """处理记录到数据库（共享逻辑）"""
    # 获取记录数据
    ...
    for hbnb_num, record_content in full_records.items():
        ...
        try:
            # 清理内容 - 先清理重复标题，再清理问题字符
            cleaned_content = processor.clean_duplicate_headers(record_content)
            ...
            if exists_info['full_record']:
                ...
                if original_info:
                    if original_info['record_content'].strip() == cleaned_content.strip():
                        # 内容完全相同，跳过
                        ...
                        db.create_full_record(hbnb_num, final_content)
    ...
    for hbnb_num, record_line in simple_records.items():
        ...
        try:
            # 清理简单记录内容
            cleaned_line = clean_hbpr_record_content(record_line, hbnb_num)
            exists_info = db.check_hbnb_exists(hbnb_num)
            if not exists_info['full_record'] and not exists_info['simple_record']:
                db.create_simple_record(hbnb_num, cleaned_line)
                stats['new_records'] += 1
        except Exception as e:
