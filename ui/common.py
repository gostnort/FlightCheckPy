...
def create_memory_database_instance() -> HbprDatabase:
    ...
    # 创建自定义的HbprDatabase实例
    db = MemoryHbprDatabase(conn)
    return db


class MemoryHbprDatabase(HbprDatabase):
    """
    专门用于内存数据库的HbprDatabase类
    """
    def __init__(self, memory_conn: sqlite3.Connection):
        """
        初始化内存数据库实例
        Args:
            memory_conn: 内存数据库连接
        """
        # 不调用父类的__init__，直接设置属性
        self._memory_conn = memory_conn
        self.db_file = ":memory:"
        self._chbpr_fields_initialized = True  # 假设内存数据库已包含所有字段
        self.stats_manager = None  # 内存数据库不需要统计缓存
    ...
    def get_hbpr_record(self, hbnb_number: int):
        ...
        try:
            ...
            raise Exception(f"Memory database error: {e}")
...
def ensure_memory_database() -> MemoryHbprDatabase:
    """
    确保有可用的内存数据库，如果没有则终止应用
    Returns:
        MemoryHbprDatabase: 内存数据库实例
    """
    current_memory_db = st.session_state.get('current_memory_db')
    ...
    try:
        ...
        # 创建并缓存实例
        db_instance = MemoryHbprDatabase(conn)
...
def get_global_database() -> MemoryHbprDatabase:
    """
    获取全局统一的内存数据库实例
    这是整个项目唯一的数据库访问入口
    Returns:
...
class GlobalDatabaseManager:
    ...
    @staticmethod
    def get_database() -> MemoryHbprDatabase:
        """获取数据库实例"""
...
def auto_save_memory_database():
    ...
    try:
        ...
        # 同步保存（避免线程问题）
        target_conn = sqlite3.connect(save_path)
        memory_conn.backup(target_conn)
        target_conn.close()
...
class MemoryHbprDatabaseWithAutoSave(MemoryHbprDatabase):
    """
    带自动保存功能的内存数据库类
    """
...
class EnhancedGlobalDatabaseManager(GlobalDatabaseManager):
    ...
    @staticmethod
    def get_database() -> MemoryHbprDatabaseWithAutoSave:
        """获取带自动保存功能的数据库实例"""
        current_memory_db = st.session_state.get('current_memory_db')
        ...
        try:
            ...
            # 创建并缓存带自动保存功能的实例
            db_instance = MemoryHbprDatabaseWithAutoSave(conn)
            st.session_state[db_instance_key] = db_instance
